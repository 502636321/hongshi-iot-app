...
<script>
  ...
  import { findAnemoclinographChart } from '@/api/structure'

    export default {
        name: "AnemoclinographItemChartTab",
        props: ['deviceId'],
        data() {
            return {
              params: {
        ...
        methods: {
          ...
          fetchChart () {
            findAnemoclinographChart(this.deviceId, { }).then(({ data }) => {
              this.option.series[0].data = data.map(item => {
                return [moment(item.timestamp).format('YYYY-MM-DD HH:mm'), item.displacementH]
              })
              this.option.series[1].data = data.map(item => {
                return [moment(item.timestamp).format('YYYY-MM-DD HH:mm'), item.displacementV]
              })
            })
          }
        },
