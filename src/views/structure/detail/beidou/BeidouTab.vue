<template>
  <div class="list-group">
    <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="fetchDevices">
      <device-item-thumb v-for="item in devices" :key="item.id" :device="item" :src="`/structure/detail/beidou-item/detail?deviceId=${item.id}&deviceName=${item.nickname}`">
        <template slot="value">
          <p>水平:{{item.lastRecordDisplacementH || 0}}mm / 垂直:{{item.lastRecordDisplacementV || 0}}mm</p>
        </template>
      </device-item-thumb>
    </van-list>
  </div>
