...
<script>
  ...
  import { findHydrostaticChart } from '@/api/structure'

    export default {
        name: "HydrostaticItemChartTab",
        props: ['deviceId'],
        data() {
            return {
              params: {
        ...
        methods: {
          ...
          fetchChart () {
            findHydrostaticChart(this.deviceId, { }).then(({ data }) => {
              this.option.series[0].data = data.map(item => {
                return [moment(item.timestamp).format('YYYY-MM-DD HH:mm'), item.difference]
              })
            })
          }
        },
