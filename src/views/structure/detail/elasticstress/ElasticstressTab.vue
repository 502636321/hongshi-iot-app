<template>
  <div class="list-group">
    <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="fetchDevices">
      <device-item-thumb v-for="item in devices" :key="item.id" :device="item" :src="`/structure/detail/elasticstress-item/detail?deviceId=${item.id}&deviceName=${item.nickname}`">
        <template slot="value">
          <p>监测值：{{item.lastRecordMeasured || 0}}kN</p>
        </template>
      </device-item-thumb>
    </van-list>
  </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        devices: [],
        DEVICE_TYPE: 'ELASTICSTRESS_TYPE',
        loading: false,
        finished: true,
      };
    },
