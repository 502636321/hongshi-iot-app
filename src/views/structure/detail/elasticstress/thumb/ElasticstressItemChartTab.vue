...
<script>
  ...
  import { findElasticstressChart } from '@/api/structure'

    export default {
        name: "ElasticstressItemChartTab",
        props: ['deviceId'],
        data() {
            return {
              params: {
        ...
        methods: {
          ...
          fetchChart () {
            findElasticstressChart(this.deviceId, { }).then(({ data }) => {
              this.option.series[0].data = data.map(item => {
                return [moment(item.timestamp).format('YYYY-MM-DD HH:mm'), item.measured]
              })
            })
          }
        },
