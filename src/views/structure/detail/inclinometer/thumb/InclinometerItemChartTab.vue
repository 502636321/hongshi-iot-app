...
<script>
  ...
  import { findInclinometerChart } from '@/api/structure'

    export default {
        name: "InclinometerItemChartTab",
        props: ['deviceId'],
        data() {
            return {
              params: {
        ...
        methods: {
          ...
          fetchChart () {
            findInclinometerChart(this.deviceId, { }).then(({ data }) => {
              this.option.series[0].data = data.map(item => {
                return [moment(item.timestamp).format('YYYY-MM-DD HH:mm'), item.angle_x]
              })
              this.option.series[1].data = data.map(item => {
                return [moment(item.timestamp).format('YYYY-MM-DD HH:mm'), item.angle_y]
              })
              this.option.series[2].data = data.map(item => {
                return [moment(item.timestamp).format('YYYY-MM-DD HH:mm'), item.angle_z]
              })
            })
          }
        },
